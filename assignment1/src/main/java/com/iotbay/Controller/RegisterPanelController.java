...
public class RegisterPanelController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)   throws ServletException, IOException { 
        ...
        try {
            ...
            if (registeredUserType.equalsIgnoreCase("customer")) {
                ...
                // doesn't replace session if current session is staff
                if (!(session.getAttribute("user") instanceof Staff)) {
                    session.setAttribute("user", customerUser); // Only set session user if it's self-registration
